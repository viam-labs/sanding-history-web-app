...
const VideoStoreSelector: React.FC<VideoStoreSelectorProps> = ({
  ...
  useEffect(() => {
    if (resources.length === 0 || !robotClient || hasAutoSelected.current)
      return
  ...
  return (
    <div className="video-store-selector">
      ...
      {error && (
        <div className="video-store-selector-message error">{error}</div>
      )}
