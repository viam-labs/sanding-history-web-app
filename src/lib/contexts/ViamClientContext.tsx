...
const ViamClientContext = createContext<ViamClientContextType | undefined>(
  undefined
)
...
export function ViamClientProvider({ children }: { children: ReactNode }) {
  ...
  const urlAndCookieData = useMemo(() => {
    ...
    if (!machineInfo) {
      errors.push(
        'Invalid URL format. Expected: /machine/[machine-name]-main.[location-id].viam.cloud'
      )
    }
    ...
    if (machineInfo) {
      ...
      if (!cookieData) {
        ...
        try {
          ...
          errors.push(
            `Failed to parse cookie data: ${error instanceof Error ? error.message : String(error)}`
          )
        }
      }
    }
    ...
    const initializeClients = async (machineId: string) => {
      ...
      try {
        ...
        setInitializationErrors((prev) => [
          ...prev,
          `Failed to create viam client: ${error instanceof Error ? error.message : String(error)}`,
        ])
        return
      }
